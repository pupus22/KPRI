import {
  firebaseConfigured, watchAuth, login, logout, checkAccess, loadData, addCustomer,
  addProduct, adjustStock, addSale, updateSale, voidSale, payDebt, getTrace, getExportData
...
let state = { tab:"dashboard", query:"", products:[],customers:[],sales:[],debts:[] };
let toastTimer;
...
function renderBody(){
  ...
  if(state.tab==="transactions"){
    ...
    const rows=state.debts.filter(s=>`${s.invoiceNo} ${s.customerName} ${s.itemSummary}`.toLowerCase().includes(q));html=rows.length?`<div class="grid-cards">${rows.map(s=>`<article class="item-card"><h3>${esc(s.customerName)}</h3><p>${esc(s.invoiceNo)} · ${dateTime(s.createdAt)}</p><button class="btn outline" data-trace-kind="transaction" data-id="${s.id}">${esc(s.itemSummary)}</button><div class="split"><span>Total<br><b>${money(s.total)}</b></span><span>Sisa bon<br><b class="error-text">${money(s.remaining)}</b></span></div><button class="btn primary" data-action="pay-debt" data-id="${s.id}">Catat pembayaran</button></article>`).join("")}</div>`:empty("Tidak ada bon aktif","Semua transaksi bon sudah lunas.");
  } else if(state.tab==="products"){
    const rows=state.products.filter(p=>`${p.sku} ${p.name} ${p.category}`.toLowerCase().includes(q));html=rows.length?`<div class="grid-cards">${rows.map(p=>`<article class="item-card"><button class="btn ghost" data-trace-kind="product" data-id="${p.id}"><b>${esc(p.name)}</b></button><p>${esc(p.sku)} · ${esc(p.category)}</p><div class="split"><span>Stok<br><b>${qty(p.stock)} ${esc(p.unit)}</b></span><span>Harga jual<br><b>${money(p.salePrice)}</b></span></div><button class="btn outline" data-action="stock" data-id="${p.id}">Sesuaikan stok</button></article>`).join("")}</div>`:empty("Barang tidak ditemukan","Tambahkan barang atau ubah pencarian.");
  } else if(state.tab==="customers"){
    const rows=state.customers.filter(c=>`${c.name} ${c.phone||""} ${c.address||""}`.toLowerCase().includes(q));html=rows.length?`<div class="table-wrap"><table><thead><tr><th>Nama</th><th>Kontak</th><th>Transaksi</th><th class="right">Sisa bon</th></tr></thead><tbody>${rows.map(c=>`<tr data-click data-trace-kind="customer" data-id="${c.id}"><td><b>${esc(c.name)}</b><small>${esc(c.address||"Alamat belum diisi")}</small></td><td>${esc(c.phone||"—")}</td><td>${c.transactionCount}</td><td class="right"><b>${money(c.debtBalance)}</b></td></tr>`).join("")}</tbody></table></div>`:empty("Pembeli tidak ditemukan","Tambah pembeli khusus transaksi bon.");
  }
  $("#body").innerHTML=html;
}

function showCustomerModal(onCreated){
  openModal(`<h2>Tambah pembeli</h2><p class="subtitle">Pembeli digunakan untuk menghubungkan transaksi dan bon.</p><form id="customerForm" class="form-stack"><label>Nama lengkap<input name="name" required></label><label>Nomor telepon<input name="phone" inputmode="tel"></label><label>Alamat<textarea name="address"></textarea></label><label>Catatan<textarea name="notes"></textarea></label><div class="actions"><button type="button" class="btn outline" data-close-modal>Batal</button><button class="btn primary">Simpan pembeli</button></div></form>`);
  $("#customerForm").addEventListener("submit",async event=>{event.preventDefault();try{const id=await addCustomer(formObject(event.currentTarget));closeModal();await refresh("Pembeli berhasil ditambahkan");if(onCreated)onCreated(id)}catch(error){toast(error.message,true)}});
}
function showProductModal(){
  openModal(`<h2>Tambah barang</h2><p class="subtitle">SKU dan stok awal menjadi titik awal pelacakan.</p><form id="productForm" class="form-stack"><div class="form-grid"><label>Kode / SKU<input name="sku" required></label><label>Nama barang<input name="name" required></label><label>Kategori<input name="category" value="Umum"></label><label>Satuan<input name="unit" value="pcs" required></label><label>Harga beli<input name="purchasePrice" type="number" min="0" required></label><label>Harga jual<input name="salePrice" type="number" min="0" required></label><label>Stok awal<input name="initialStock" type="number" min="0" step="any" value="0" required></label><label>Stok minimum<input name="minStock" type="number" min="0" step="any" value="0" required></label></div><div class="actions"><button type="button" class="btn outline" data-close-modal>Batal</button><button class="btn primary">Simpan barang</button></div></form>`);
  $("#productForm").addEventListener("submit",async event=>{event.preventDefault();try{await addProduct(formObject(event.currentTarget));closeModal();await refresh("Barang berhasil disimpan")}catch(error){toast(error.message,true)}});
}
...
function showSaleModal(saleId=""){
  if(!state.products.length){toast("Tambahkan barang terlebih dahulu",true);return}
  ...
  const sale=editing?{
    ...
    lines:(existing.items||[]).map(item=>({id:item.id,productId:item.productId,qty:item.qty,unitPrice:item.unitPrice}))
  ...
  const total=()=>sale.lines.reduce((sum,line)=>{const p=state.products.find(x=>x.id===line.productId);return sum+Number(line.qty||0)*Number(line.unitPrice||p?.salePrice||0)},0);
  ...
  const draw=()=>{
    ...
    const productOptions=line=>state.products.map(p=>`<option value="${p.id}" ${line.productId===p.id?"selected":""}>${esc(p.name)} · stok ${qty(p.stock)} ${esc(p.unit)}</option>`).join("");
    const customerOptions=state.customers.map(c=>`<option value="${c.id}" ${sale.customerId===c.id?"selected":""}>${esc(c.name)}</option>`).join("");
    ...
    document.querySelectorAll("[data-line]").forEach(input=>input.onchange=e=>{const line=sale.lines[Number(e.target.dataset.line)];line[e.target.dataset.field]=e.target.value;if(e.target.dataset.field==="productId")line.unitPrice=state.products.find(p=>p.id===e.target.value)?.salePrice||"";draw()});
