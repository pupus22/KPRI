...
function showSaleModal(saleId=""){
  ...
  const total=()=>sale.lines.reduce((sum,line)=>{const p=catalogProducts.find(x=>x.id===line.productId);return sum+Number(line.qty||0)*Number(line.unitPrice||p?.salePrice||0)},0);
  ...
  const draw=()=>{
    ...
    const productOptions=line=>catalogProducts.map(p=>`<option value="${p.id}" ${line.productId===p.id?"selected":""}>${esc(p.name)}${p.archived?" (dihapus)":""} · stok ${qty(p.stock)} ${esc(p.unit)}</option>`).join("");
